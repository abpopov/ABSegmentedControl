...
@interface ABSegmentedControl () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) ABGradientView *gradientView;
...
- (void)initDefaults {
    ...
    self.shadowHideDuration = 1.5;
    self.shadowShowDuration =1.5;
}
...
- (void)initGradientView {
    ...
    self.gradientView = [[ABGradientView alloc] initWithFrame:CGRectZero];
...
- (void)setupColorsArray:(NSMutableArray *)colors withCount:(NSInteger)count {
    for (NSInteger index = -1; index <= count; index++) {
        ...
        if (index == -1) {
            stateColors = [self gradientColorForBounce:ABSegmentedControlBounceLeft];
        }
        if (index >= 0 && index < count) {
            stateColors = [self gradientColorForStateAtIndex:index];
        }
        if (index == count) {
            stateColors = [self gradientColorForBounce:ABSegmentedControlBounceRight];
        }
...
- (NSArray <UIColor *> *)gradientColorForBounce:(ABSegmentedControlBounce)bounce {
...
- (void)removeOldConstraints {
    [self AB_removeConstraintsFromSubTree:[NSSet setWithArray:self.addedConstraintsToRemove]];
...
#pragma mark - Setters
- (void)setDataSource:(id<ABSegmentedControlDataSource>)dataSource {
...
- (void)setTransitionStyle:(ABSegmentedControlTransitionStyle)transitionStyle {
...
- (CGFloat)gradientLocationForBounce:(ABSegmentedControlBounce)bounce colorIndex:(NSInteger)index colorsCount:(NSInteger)colorsCount statesCount:(NSInteger)statesCount {
    CGFloat totalCount = statesCount + 2 + (statesCount + 1) * self.gradientBackVelocity;
    switch (bounce) {
        case ABSegmentedControlBounceLeft:
            return index * 1.0 / (colorsCount - 1) / totalCount;
            break;
            
        case ABSegmentedControlBounceRight:
...
- (CGPathRef)pathForSelectorViewFromPercentage:(CGFloat)percent {
    switch (self.shapeStyle) {
        case ABSegmentedControlShapeStyleRoundedRect:
            return [self roundedRectPathForSelectorViewFromPercentage:percent].CGPath;
            
        case ABSegmentedControlShapeStyleLiquid:
